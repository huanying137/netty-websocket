...
/**
 * @author baiyu
 * @data 2019-12-25 9:51
 *
 * 以下处理类虽然做了注释，但是在这里还是详细讲解下。
 *
 * 这个类是单例的，每个线程处理会新实例化一个类。
 * 每个成功的线程访问顺序：channelActive（开启连接）-handleHttpRequest（http握手处理）-messageReceived（消息接收处理）
 * -handlerWebSocketFrame（实际处理，可以放到其他类里面分业务进行）
 * 注意：这个demo中我做了路由功能，在handleHttpRequest中对每个channel连接的时候对每个连接的url进行绑定参数，
 * 然后在messageReceived中获取绑定的参数进行分发处理（handlerWebSocketFrame或handlerWebSocketFrame2），
 * 同时也获取了uri后置参数，有注释。
...
public class MyWebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    /**
     * channel 通道 action 活跃的 当客户端主动链接服务端的链接后，这个通道就是活跃的了。也就是客户端与服务端建立了
     * 通信通道并且可以传输数据
     */
    @Override
    ...
    /**
     * channel 通道 Inactive 不活跃的 当客户端主动断开服务端的链接后，这个通道就是不活跃的。也就是说客户端与服务端
     * 关闭了通信通道并且不可以传输数据
     */
    @Override
